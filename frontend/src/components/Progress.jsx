...
export const Progress = ({ user }) => {
    ...
    if (status === "started" || (status === "passed" && question > 1)) {
        return(
            <div className="cmd-group">
                ...
                <br />
                <ProgressBar level={level} questionLeft={questionLeft} totalLevelQuestions={totalLevelQuestions}/>
            </div>
        )    
    }
  };
  ...
  const ProgressBar = ({level, questionLeft, totalLevelQuestions}) => {
    ...
    for(let i = 1; i < 6; i++) {
        if(i<level) {
            elements.push(
                <p key={nanoid()} className="term-green" >{`Level ${i}: `} <span style={style}>{`100%`}</span>{`[${"=".repeat(maxbar)}]`}</p>
            )
        } else {
            ...
            elements.push(
                <p key={nanoid()} >{`Level ${i}: `} <span style={style}>{`${progress}%`}</span>{`[${"=".repeat(maxbar-remainBar)}${".".repeat(remainBar)}]`}</p>
            )
        }
    }
